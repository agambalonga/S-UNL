...
Il benchmark \textit{MUSE} (\emph{Machine Unlearning Six-Way Evaluation}) \cite{shi2025muse} è stato introdotto come un framework di valutazione organico e multidimensionale per analizzare l’efficacia delle tecniche di machine unlearning nei modelli di linguaggio di grandi dimensioni.  
...
Accanto alle aspettative dei data owner, MUSE tiene conto anche delle esigenze dei deployer dei modelli. In questo contesto, la quarta proprietà riguarda il \textbf{mantenimento delle prestazioni (\textit{utility preservation})}: il processo di unlearning non dovrebbe compromettere in modo significativo le prestazioni del modello sui dati di \emph{retain}.
...
\begin{itemize}
    ...
    \item $\mathrm{PrivLeak} > 0$: \emph{over-unlearning}, il modello ha rimosso eccessivamente le informazioni, trattando i dati del forget set come più estranei rispetto a quanto farebbe un modello che non li ha mai visti.
    \item $\mathrm{PrivLeak} < 0$: \emph{under-unlearning}, il modello conserva ancora tracce riconoscibili dei dati da dimenticare, risultando più vulnerabile ad attacchi inferenziali rispetto al modello di riferimento.
\end{itemize}
