...
def load_results_by_paraphrases(
    base_dir: str = "saves/unlearn", paraphrase_counts: List[int] = [0, 5, 10, 15, 20]
) -> pd.DataFrame:
    """
    ...
    for trainer in TRAINERS:
        for num_para in paraphrase_counts:
            ...
            try:
                ...
                print(
                    f"✅ Loaded: {trainer} para={num_para} (step={best_step}, last checkpoint)"
                )
        ...
        for num_para in paraphrase_counts:
            ...
            for target_epoch in epoch_checkpoints:
                ...
                if checkpoint_index >= len(checkpoint_list):
                    print(
                        f"⚠️  Missing: {trainer} para={num_para} epoch={target_epoch} "
                ...
                try:
                    ...
                    print(
                        f"✅ Loaded: {trainer} para={num_para} epoch={target_epoch} (step={step}, checkpoint #{target_epoch})"
                    )
...
def plot_metric_by_paraphrases(df: pd.DataFrame, metric: str, output_dir: str):
    ...
    plt.title(
        f"{METRIC_LABELS.get(metric, metric)} vs Number of Paraphrases (Final Epoch)",
        fontsize=15,
        fontweight="bold",
    )
...
def plot_metric_by_epochs(
    df: pd.DataFrame, metric: str, num_paraphrases: int, output_dir: str
):
    """Plotta metrica vs epoche per una configurazione di parafrasi."""
    os.makedirs(output_dir, exist_ok=True)

    metric_df = df[
        (df["metric"] == metric) & (df["num_paraphrases"] == num_paraphrases)
    ]
    ...
    plt.ylabel(METRIC_LABELS.get(metric, metric), fontsize=13)
    plt.title(
        f"{METRIC_LABELS.get(metric, metric)} vs Epochs ({num_paraphrases} Paraphrases)",
        fontsize=15,
        fontweight="bold",
    )
    plt.legend(loc="best", fontsize=11, framealpha=0.9)
...
def plot_all_paraphrases_grid(df: pd.DataFrame, output_dir: str):
    ...
    plt.suptitle(
        "TOFU Metrics vs Number of Paraphrases (Final Epoch)",
        fontsize=16,
        fontweight="bold",
        y=0.995,
    )
...
def plot_all_epochs_grid(df: pd.DataFrame, num_paraphrases: int, output_dir: str):
    ...
    plt.suptitle(
        f"TOFU Metrics vs Epochs ({num_paraphrases} Paraphrases)",
        fontsize=16,
        fontweight="bold",
        y=0.995,
    )
...
def generate_summary_tables(
    df_para: pd.DataFrame, df_epochs: pd.DataFrame, output_dir: str
):
    ...
    if not df_para.empty:
        summary_para = df_para.pivot_table(
            index=["trainer", "num_paraphrases"], columns="metric", values="value"
        ).reset_index()
    ...
    if not df_epochs.empty:
        for num_para in df_epochs["num_paraphrases"].unique():
            summary_epochs = (
                df_epochs[df_epochs["num_paraphrases"] == num_para]
                .pivot_table(
                    index=["trainer", "epoch"], columns="metric", values="value"
                )
                .reset_index()
            )
