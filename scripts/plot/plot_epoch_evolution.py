...
METRICS_CONFIG = {
    ...
    "model_utility": {"label": "Model Utility", "direction": "higher", "color": "GnBu"},
        ...
        for num_para in PARAPHRASE_CONFIGS:
            ...
            for target_epoch in target_epochs:
                ...
                if abs(actual_epoch - target_epoch) > 0.5:
                    print(
                        f"⚠️  No checkpoint near epoch {target_epoch}: {method} para={num_para}"
                    )
                    continue
                ...
                try:
                    ...
                    if target_epoch == target_epochs[0]:
                        print(
                            f"✅ {method} para={num_para}: found {len(checkpoint_dirs)} checkpoints"
                        )
...
def plot_method_evolution(df, method, metric, output_dir):
    ...
    ax.set_title(
        f"{method}: Evoluzione {config['label']} attraverso le Epoche",
        fontsize=18,
        fontweight="bold",
        pad=20,
    )
    ...
    ax.text(
        0.02,
        0.98,
        direction_text[config["direction"]],
...
def plot_all_methods_comparison(df, metric, epoch, output_dir):
    ...
    for i, para_count in enumerate(PARAPHRASE_CONFIGS):
        ...
        values = [
            para_data[para_data["trainer"] == m][metric].values[0]
            if len(para_data[para_data["trainer"] == m]) > 0
            else 0
            for m in METHODS
        ]
    ...
    ax.set_title(
        f"Confronto {config['label']} tra Metodi (Epoca {epoch})",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_convergence_heatmap(df, method, metric, output_dir):
    ...
    pivot_data = method_data.pivot(
        index="epoch", columns="num_paraphrases", values=metric
    )
    ...
    ax.set_title(
        f"{method}: Convergenza {config['label']}",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_delta_analysis(df, method, metric, output_dir):
    ...
    for epoch in EPOCHS:
        ...
        for para_count in PARAPHRASE_CONFIGS[1:]:  # Escludi para0
            para_value = epoch_data[epoch_data["num_paraphrases"] == para_count][
                metric
            ].values[0]
            delta_pct = (
                ((para_value - baseline) / baseline) * 100 if baseline != 0 else 0
            )
            deltas.append(delta_pct)

        ax.plot(
            PARAPHRASE_CONFIGS[1:],
            deltas,
            marker="o",
            linewidth=2,
            markersize=8,
    ...
    ax.set_title(
        f"{method}: Impatto Parafrasi su {config['label']}",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
        ...
        for method in METHODS:
            ...
            for para in PARAPHRASE_CONFIGS:
                ...
                if len(values) >= 2:
                    ...
                    f.write(
                        f"  para{para}: {values[0]:.3f} (ep5) → {values[-1]:.3f} (ep20) "
                        f"[{improvement:+.1f}%]\n"
                    )
            ...
            for para in PARAPHRASE_CONFIGS:
                para_data = method_data[
                    method_data["num_paraphrases"] == para
                ].sort_values("epoch")
                if len(para_data) >= 2:
                    ...
                    if total_improvement > 0.01:
                        ...
                        converge_epoch = None
                        for idx, (ep, val) in enumerate(
                            zip(para_data["epoch"], em_values)
                        ):
                            if val <= target_value:
                                converge_epoch = ep
                                break
                        if converge_epoch:
                            f.write(
                                f"  para{para}: Raggiunge 90% miglioramento a epoca {converge_epoch}\n"
                            )
                    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Analizza evoluzione metriche attraverso epoche"
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    parser.add_argument(
        "--methods", nargs="+", default=METHODS, help="Metodi da analizzare"
    )
    parser.add_argument(
        "--metrics",
        nargs="+",
    ...
    parser.add_argument(
        "--epochs", nargs="+", type=int, default=EPOCHS, help="Epoche da analizzare"
    )
