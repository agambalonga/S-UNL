...
def load_all_metrics(base_dir: str = "saves/unlearn",
                     paraphrase_counts: List[int] = [0, 5, 10, 15, 20],
                     target_epochs: List[int] = [20]) -> pd.DataFrame:
    """
        ...
        for num_para in paraphrase_counts:
            ...
            for target_epoch in target_epochs:
                ...
                if abs(actual_epoch - target_epoch) > 0.5:
                    print(f"⚠️  No checkpoint near epoch {target_epoch}: {trainer} para={num_para}")
                    continue
                ...
                try:
                    ...
                    print(f"✅ Loaded: {trainer} para={num_para} epoch={target_epoch} (step={actual_step}, {len([k for k in data.keys() if k not in AGGREGATE_METRICS])} individual metrics)")
...
def plot_metric_histogram(df: pd.DataFrame, trainer: str, metric: str, epoch: int, output_dir: str):
    """
    ...
    metric_df = df[(df['trainer'] == trainer) & (df['metric'] == metric) & (df['epoch'] == epoch)]
    ...
    plt.title(f'{trainer} (Epoch {epoch}): {metric_label} vs Paraphrases',
...
def plot_metric_comparison_across_trainers(df: pd.DataFrame, metric: str, epoch: int, output_dir: str):
    """
    ...
    for idx, trainer in enumerate(TRAINERS):
        trainer_df = metric_df[metric_df['trainer'] == trainer].sort_values('num_paraphrases')
    ...
    plt.xlabel('Number of Paraphrases', fontsize=13, fontweight='bold')
    plt.ylabel(metric_label, fontsize=13, fontweight='bold')
    plt.title(f'All Methods (Epoch {epoch}): {metric_label} vs Paraphrases',
              fontsize=15, fontweight='bold', pad=20)
...
def plot_all_metrics_for_trainer(df: pd.DataFrame, trainer: str, epoch: int, output_dir: str):
    """
    ...
    metric_labels = [METRIC_NAMES.get(m, m.replace('_', ' ').title()) for m in all_metrics]
    ax.set_xticks(x)
    ax.set_xticklabels(metric_labels, rotation=45, ha='right', fontsize=9)
    
    ax.set_xlabel('Metrics', fontsize=13, fontweight='bold')
    ax.set_ylabel('Value', fontsize=13, fontweight='bold')
    ax.set_title(f'{trainer} (Epoch {epoch}): All Metrics vs Paraphrases',
...
def plot_all_metrics_heatmap(df: pd.DataFrame, trainer: str, epoch: int, output_dir: str):
    """
    ...
    pivot_df = trainer_df.pivot(
        index='metric',
        columns='num_paraphrases',
        values='value'
    )
    
    # Ordina metriche per categoria
    memorization_metrics = [m for m in pivot_df.index if m in [
        'exact_memorization', 'extraction_strength', 'forget_Q_A_PARA_Prob',
        'forget_Q_A_Prob', 'forget_Q_A_ROUGE', 'forget_truth_ratio'
    ]]
    privacy_metrics = [m for m in pivot_df.index if m in [
        'mia_loss', 'mia_min_k', 'mia_min_k_plus_plus', 'mia_zlib',
        'forget_quality', 'privleak'
    ]]
    utility_metrics = [m for m in pivot_df.index if m in [
    ...
    # Formatta labels
    metric_labels = [METRIC_NAMES.get(m, m.replace('_', ' ').title()) for m in pivot_df.index]
    ...
    ax.set_title(f'{trainer} (Epoch {epoch}): Metrics Heatmap (normalized by row)',
                 fontsize=15, fontweight='bold', pad=20)
...
def generate_summary_table(df: pd.DataFrame, output_dir: str):
    ...
    summary = df.pivot_table(
        index=['trainer', 'epoch', 'num_paraphrases'],
        columns='metric',
        values='value'
    ).reset_index()
...
def main():
    ...
    parser.add_argument('--base-dir', default='saves/unlearn',
                       help='Base directory for results')
    parser.add_argument('--output-dir', default='plots/ablation_study_detailed',
    ...
    if not args.skip_comparisons:
        ...
        for epoch in sorted(df['epoch'].unique()):
            print(f"\n   === Epoch {epoch} ===")
            for metric in all_metrics:
                plot_metric_comparison_across_trainers(df, metric, epoch, args.output_dir)
                print(f"   ✅ {metric}")
    ...
    print(f"   - {args.output_dir}/summary_all_detailed_metrics.csv  (full data table with epochs)")
